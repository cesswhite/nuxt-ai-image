...
- **Image generation:** Studio uses **`useGenerateImage`** → **`POST /api/image/<slug>`** (see `app/utils/imageApiRoutes.ts`). Logic lives in **`server/api/image/*.post.ts`** (small shared helpers: **`server/utils/imageApiCommon.ts`**). **Enhance / Surprise** use **`POST /api/text/*`** (`app/utils/textApiRoutes.ts`) → **`server/utils/promptAssistOpenAi.ts`**. Allowed `model` IDs: `app/utils/studioImageModels.ts`.
